...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> handlerInsufficientProductException(InsufficientProductException err){
        return ResponseEntity.status(HttpStatus.CONFLICT).body(ApiResponse.error(err.getMessage()));
    }
    ...
    public ResponseEntity<ApiResponse<Object>> handlerInvalidUserCredentialException(InvalidUserCredentialException err){
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(ApiResponse.error(err.getMessage()));
    }

    @ExceptionHandler(PasswordMatchException.class)
    public ResponseEntity<ApiResponse<Object>> handlerPasswordMatchException(PasswordMatchException err){
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ApiResponse.error(err.getMessage()));
    }
    ...
    public ResponseEntity<ApiResponse<Object>> handleMailException(MailException err){
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.error("Email not sent!"));
    }

    @ExceptionHandler(SQLException.class)
    public ResponseEntity<ApiResponse<Object>> handleSQLException(SQLException err){
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.error("SQL Exception!"));
    }
